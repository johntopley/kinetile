...
function drawTitle(ctx, logo) {
  ...
  ctx.fillText("ITERATION 3 — LIVES", WIDTH / 2, 430);
  ctx.font = "18px 'Segoe UI', sans-serif";
  ctx.fillText("Three chances. Miss the ball and a life is gone.", WIDTH / 2, 478);
...
function hudHint(game) {
  ...
  return "Hit the ball with the edge of the bat to steer it";
}
  ...
  return {
    draw(game) {
      ...
      if (game.state === STATES.GAME_OVER) {
        ...
        ctx.font = "18px 'Segoe UI', sans-serif";
        ctx.fillText("Press Space or Escape for the title screen", WIDTH / 2, HEIGHT / 2 + 36);
      }
    }
  };
}
