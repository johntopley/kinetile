...
function drawTitle(ctx, logo) {
  ...
  ctx.fillText("ITERATION 4 — THE WALL", WIDTH / 2, 430);
  ctx.font = "18px 'Segoe UI', sans-serif";
  ctx.fillText("Eight rows of bricks. Clear them all.", WIDTH / 2, 478);
...
function hudHint(game) {
  ...
  if (game.state === STATES.LEVEL_CLEAR) {
    return "Wall cleared — press Space for another";
  }
  ...
  return {
    draw(game) {
      ...
      if (game.state === STATES.GAME_OVER || game.state === STATES.LEVEL_CLEAR) {
        ...
        ctx.fillText(game.state === STATES.GAME_OVER ? "GAME OVER" : "WALL CLEARED", WIDTH / 2, HEIGHT / 2);
        ctx.font = "18px 'Segoe UI', sans-serif";
        ctx.fillText(
          game.state === STATES.GAME_OVER
            ? "Press Space or Escape for the title screen"
            : "Press Space to rebuild the wall",
          WIDTH / 2,
          HEIGHT / 2 + 36
        );
      }
