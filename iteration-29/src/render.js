...
import { pauseCaptions, titleCaptions } from "./save.js";
import { titleCaptionOpacity, titleLogoPose } from "./title.js";

function drawLogo(ctx, logo, pose) {
  if (!logo || !logo.complete || logo.naturalWidth === 0) {
    return { width: 0, height: 0, x: 0, y: pose.y };
  }

  const baseScale = Math.min(1, 720 / logo.naturalWidth);
  const width = logo.naturalWidth * baseScale * pose.scale;
  const height = logo.naturalHeight * baseScale * pose.scale;
  ...
  glow.addColorStop(0, `rgba(90, 214, 255, ${0.22 + pose.glow / 180})`);
  glow.addColorStop(1, "rgba(90, 214, 255, 0)");
  ctx.fillStyle = glow;
  ctx.fillRect(x - 40, y - 20, width + 80, height + 40);
  ctx.drawImage(logo, x, y, width, height);
  ctx.restore();
  return { width, height, x, y };
}

function drawTitle(ctx, logo, time, hasSave) {
  ctx.fillStyle = COLOURS.background;
  ctx.fillRect(0, 0, WIDTH, HEIGHT);

  const pose = { ...titleLogoPose(time), time };
  const box = drawLogo(ctx, logo, pose);

  const caption = titleCaptionOpacity(time);
  ctx.save();
  ctx.globalAlpha = caption;
  ctx.fillStyle = COLOURS.muted;
  ctx.textAlign = "center";
  ctx.font = "18px 'Segoe UI', sans-serif";
  titleCaptions(hasSave).forEach((line, index) => {
    ctx.fillText(line, WIDTH / 2, box.y + box.height + 36 + index * 26);
  });
  ctx.restore();
}
...
function drawHud(ctx, game) {
  ...
  ctx.textAlign = "right";
  ctx.fillText(`LEVEL ${game.level}`, WIDTH - 20, 36);
  ctx.textAlign = "center";
  ctx.fillText(
...
function hudHint(game) {
  if (game.state === STATES.PAUSED) {
    return game.saveNotice
      ? "Game saved · P resume · Esc title"
      : "Paused · P resume · S save · Esc title";
  }
  if (game.state === STATES.SERVE) {
    return "Press Space to serve · P to pause";
  }
  ...
  if (game.effects.slow > 0) {
    return `Slow ball  ${game.effects.slow.toFixed(1)}s`;
  }
  ...
  return "Catch falling capsules · P to pause";
}
...
function drawOverlay(ctx, game) {
  ...
  if (game.state === STATES.PAUSED) {
    ctx.fillText("PAUSED", WIDTH / 2, HEIGHT / 2 - 36);
    ctx.font = "18px 'Segoe UI', sans-serif";
    ctx.fillStyle = COLOURS.muted;
    pauseCaptions(game.saveNotice).forEach((line, index) => {
      ctx.fillText(line, WIDTH / 2, HEIGHT / 2 + 8 + index * 26);
    });
    return;
  }
  ...
  return {
    draw(game, elapsed = 0) {
      ...
      if (game.state === STATES.TITLE) {
        if (lastState !== STATES.TITLE) {
          titleTime = 0;
        }
        titleTime += Math.max(0, elapsed);
        drawTitle(ctx, assets.logo, titleTime, game.hasSave);
        ctx.save();
        ctx.globalAlpha = titleCaptionOpacity(titleTime);
