...
describe("game", () => {
  ...
  it("clears the wall when the last brick is hit", () => {
    const game = createGame();
    startMatch(game);
    launchBall(game);
    ...
    game.ball.vy = -200;
    step(game, 0);
    expect(remainingBricks(game.bricks)).toHaveLength(0);
    expect(game.state).toBe(STATES.LEVEL_CLEAR);
  });

  it("advances to a faster new level after a clear", () => {
    const input = createInput();
    const game = createGame({ input, seed: 7 });
    startMatch(game);
    const firstLayout = game.bricks.map((brick) => `${brick.col},${brick.row}`).join("|");
    game.state = STATES.LEVEL_CLEAR;
    input.press("Space");
    step(game, FIXED_DT);
    expect(game.level).toBe(2);
    expect(game.state).toBe(STATES.SERVE);
    expect(game.ball.speed).toBe(speedForLevel(2));
    const secondLayout = game.bricks.map((brick) => `${brick.col},${brick.row}`).join("|");
