...
export function mirroredPaddle(paddle) {
  return {
    ...paddle,
    x: WIDTH - paddle.x
  };
...
export function activePaddles(paddle, twin) {
  return twin ? [paddle, mirroredPaddle(paddle)] : [paddle];
}

export function updatePaddle(paddle, input, dt, playfield, invert = false) {
  let dir = 0;
  if (input.left()) {
    dir -= 1;
  }
