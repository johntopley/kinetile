...
function drawTitle(ctx, logo) {
  ...
  ctx.fillText("ITERATION 16 — CATCH", WIDTH / 2, 400);
  ctx.font = "18px 'Segoe UI', sans-serif";
  ctx.fillStyle = COLOURS.muted;
  ctx.fillText("A pale capsule makes the bat sticky. Space throws the ball back.", WIDTH / 2, 432);
...
function drawPaddle(ctx, paddle, effects = {}) {
  ...
  const colour = effects.laser > 0 ? COLOURS.powerUps.LASER : COLOURS.paddle;
  ctx.save();
...
function drawBall(ctx, ball, piercing) {
  const colour = piercing ? COLOURS.powerUps.PIERCE : COLOURS.ball;
  ctx.save();
