...
function drawTitle(ctx, logo) {
  ...
  ctx.fillText("ITERATION 14 — SHIFTING TILES", WIDTH / 2, 400);
  ctx.font = "18px 'Segoe UI', sans-serif";
  ctx.fillStyle = COLOURS.muted;
  ctx.fillText("Some tiles slip into a neighbouring gap when first hit.", WIDTH / 2, 432);
...
function drawBall(ctx, ball, piercing) {
  ctx.fillStyle = piercing ? COLOURS.powerUps.PIERCE : COLOURS.ball;
  ctx.shadowColor = piercing ? COLOURS.powerUps.PIERCE : "#ff6ad5";
...
function drawBricks(ctx, bricks) {
  for (const brick of bricks) {
    ...
    ctx.globalAlpha = 0.45 + 0.55 * strength;
    ctx.fillStyle = brick.shifting && !brick.shifted ? COLOURS.powerUps.SLOW : brickColour(brick);
    ctx.fillRect(origin.x, origin.y, brick.width, brick.height);
    ctx.globalAlpha = 1;
    ...
    if (brick.maxHits > 1) {
      ...
      for (let pip = 0; pip < brick.hits; pip += 1) {
        ctx.fillRect(origin.x + 6 + pip * 8, origin.y + brick.height / 2 - 2, 5, 4);
      }
    }
  }
}
...
function drawPlayfield(ctx, game) {
  ...
  for (const ball of game.balls) {
    drawBall(ctx, ball, game.effects.pierce > 0);
  }
}
