...
function drawTitle(ctx, logo) {
  ...
  ctx.fillText("ITERATION 13 — ARMOURED BRICKS", WIDTH / 2, 400);
  ctx.font = "18px 'Segoe UI', sans-serif";
  ctx.fillStyle = COLOURS.muted;
  ctx.fillText("Some tiles take two or three hits before they vanish.", WIDTH / 2, 432);
  ...
  for (const brick of bricks) {
    ...
    if (brick.maxHits > 1) {
      ...
      for (let pip = 0; pip < brick.hits; pip += 1) {
        ctx.fillRect(brick.x + 6 + pip * 8, brick.y + brick.height / 2 - 2, 5, 4);
      }
    }
  }
}
