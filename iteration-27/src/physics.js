...
export function attractBallToPaddle(ball, paddle, dt) {
  if (ball.y < MAGNET_MIN_Y) {
    return false;
  }
  const dx = paddle.x - ball.x;
  if (Math.abs(dx) > MAGNET_RANGE || dx === 0) {
    return false;
  }
  setBallVelocity(ball, ball.vx + Math.sign(dx) * MAGNET_PULL * dt, ball.vy);
  return true;
}
