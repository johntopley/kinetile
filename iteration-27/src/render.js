...
function drawTitle(ctx, logo) {
  ...
  ctx.fillText("ITERATION 27 — BALL SLOW", WIDTH / 2, 400);
  ctx.font = "18px 'Segoe UI', sans-serif";
  ctx.fillStyle = COLOURS.muted;
  ctx.fillText("A pale capsule slows the ball, not the bat.", WIDTH / 2, 432);
...
function hudHint(game) {
  ...
  if (game.effects.ballSlow > 0) {
    return `Ball slow  ${game.effects.ballSlow.toFixed(1)}s`;
  }
  if (game.gateOpen) {
